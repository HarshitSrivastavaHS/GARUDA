module.exports = {
    ...
    async execute(message, args, bot, Discord, prefix) {
        ...
        const mentionMessage = message.mentions.members.first()&&message.mentions.members.filter(m=>args[0]&&args[0].includes(m.user.id)).size>=1?message.mentions.members.filter(m=>args[0]&&args[0].includes(m.user.id)).first(): undefined|| args[0];bers.first();
