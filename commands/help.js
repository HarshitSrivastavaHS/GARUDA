...
module.exports = {
    ...
    async execute(message, args, bot, Discord, prefix) {
        ...
        if (commandFiles.includes(args[0]+".js")) {
           ...
           helpembed.addFields({name:`Name`, value: `${command.name}`},{name:`Description`, value: `${command.description}`},{name:`Usage`, value: `${command.usage?command.usage.replace(PREFIX_REG, prefix):"not added"}`}, {name:`Permissions Required by bot`, value: `${command.permissions?command.permissions.join(", ").toLowerCase().replace(/_/g," "):"not added"}`});
        }
        else {
