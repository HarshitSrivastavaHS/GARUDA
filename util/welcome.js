...
module.exports = {
    async execute(member, welcomeCH, Discord) {
      ...
      ctx.fillText(`Humans: ${member.guild.members.cache.filter(m=>!m.user.bot).size} Bots: ${member.guild.members.cache.filter(m=>m.user.bot).size}`, 240+230-ctx.measureText(`Member #${member.guild.members.cache.filter(m=>!m.user.bot).size} Bots: ${member.guild.members.cache.filter(m=>m.user.bot).size}`).width/2, canvas.height / 3.5 + canvas.height / 1.8 - 10);
      ctx.beginPath();
