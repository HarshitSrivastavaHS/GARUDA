...
module.exports = {
    async execute(member, welcomeCH, greeting) {
      ...
      welcomeCH.send({content:greeting?greeting:`Welcome to **${member.guild.name}**, <@${member.user.id}>`, files:[attachment], allowedMentions: { parse: ['users']}}).catch(e=>{console.log(e)});
    }
}
